...
directory <- args[1]
#create output dir
output_dir <- paste0("./", directory, "_rlog_transformed")
...
for (filename in dir_files){
  ...
  #rlog transform
  rlog_trans <- rlog(count_data)
  #rlog loses rownames, so put them back
  rownames(rlog_trans) <- rownames(count_data)
  #write to file
  rlog_trans %>% 
    as.data.frame() %>% 
    rownames_to_column("gene") %>% 
    write_delim(paste0(output_dir, "/", gsub("\\.pcl$", "_rlog_trans.pcl", file_basename)),
                delim = "\t",
                col_names = T)
}
